...
#      pending "Implement the bonus section of the README. Then remove this line."
      ...
      pikachu.alter_hp(59)
      expect(@db.execute("SELECT hp FROM pokemon WHERE id = 1;").flatten.first).to eq(59)
    end

    # Now we alter Magikarp's hp
    it "alters Magikarp's hp" do
#      pending "Implement the bonus section of the README. Then remove this line."
      magikarp = Pokemon.new(1, "Magikarp", "karp", @db)
      magikarp.alter_hp(0)
      expect(@db.execute("SELECT hp FROM pokemon WHERE id = 1;").flatten.first).to eq(0)
    end
  end
end
