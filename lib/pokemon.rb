...
class Pokemon
  ...
  def initialize(id=nil, pk_name=nil, pk_type=nil, db=nil, hp=nil)
    @id = id
    @name = pk_name
    @type = pk_type
    @da = db
  ...
  def self.save(pk_name, pk_type, db, hp=60)
    db.execute("INSERT INTO pokemon (name, type, hp) VALUES (?, ?, ?)", pk_name, pk_type, hp)
  end
